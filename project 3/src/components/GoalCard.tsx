...
import { calculateEstimatedCompletionDate } from '../utils/dateUtils';
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
      <div className="flex justify-between items-start">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-600">
            1回の目標: {goal.dailyTarget.toFixed(0)} / 総目標: {goal.targetAmount}
          </p>
          <p className="text-sm text-gray-600">
            頻度: {goal.frequency === 1 ? '毎日' : `${goal.frequency}日に1度`}
          </p>
          <p className="text-sm text-gray-600">
            目的: {goal.purpose}
          </p>
      ...
      <div className="text-right text-sm text-gray-600">
        <div>開始日: {new Date(goal.startDate).toLocaleDateString()}</div>
        <div>目標日: {new Date(goal.endDate).toLocaleDateString()}</div>
        <div>予定終了日: {estimatedCompletionDate}</div>
      </div>
